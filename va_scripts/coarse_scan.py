# -*- coding: utf-8 -*-
"""VA 粗扫：起点不触发、终点触发，按帧顺序保存位置（um）和清晰度。
...
def initialize_coarse_scan(start_position, end_position, step):
    """开始新一轮粗扫：返回空记录，不读取清晰度、不记录起点。"""
    return _dump(_new_data(*_scan_parameters(start_position, end_position, step)))


def append_coarse_sharpness(sharpness, start_position, end_position, step,
    ...
    不兼容旧版仅含 scores 的数据，首次使用或重新粗扫时需显式初始化。
    """
    ...
    if not isinstance(data, dict) or not isinstance(data.get("samples"), list):
        raise ValueError("历史数据缺少位置采样列表 samples，请先初始化本轮粗扫")
    for key in ("schema_version", "scan_type", "position_unit", "start_position",
                "end_position", "step", "expected_count"):
        if isinstance(data.get(key), bool) or data.get(key) != template[key]:
            raise ValueError("历史数据格式或扫描参数发生变化，请先初始化本轮粗扫：" + key)

    samples = data["samples"]
    if len(samples) >= expected_count:
        raise ValueError("已达到预期采样数，禁止继续追加；新一轮粗扫请先初始化")
